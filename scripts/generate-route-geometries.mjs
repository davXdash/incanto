...
async function routeGeometry(points){const coords=points.map(([lat,lng])=>`${lng},${lat}`).join(';');const url=`https://router.project-osrm.org/route/v1/driving/${coords}?overview=full&geometries=geojson&steps=false`;
  const response=await fetch(url,{headers:{'User-Agent':'IncantoRouteGenerator/1.0'}});if(!response.ok)throw new Error(`OSRM ${response.status}`);const data=await response.json();if(data.code!=='Ok'||!data.routes?.[0]?.geometry?.coordinates)throw new Error(data.code||'No route');return data.routes[0].geometry.coordinates.map(([lng,lat])=>[lat,lng]);}

async function main(){const [routes,coordinates]=await Promise.all([readRoutes(),readCoordinates()]);const output={generatedAt:new Date().toISOString(),provider:'OSRM/OpenStreetMap',routes:{}};
  for(const route of routes){const mapped=route.stops.map(name=>({name,coord:coordinates[normalize(name)]||null})).filter(item=>item.coord);if(mapped.length<2)continue;
    let road=[];let mode='road';
    try{road=await routeGeometry(mapped.map(item=>item.coord));}
    catch(error){console.warn(`${route.id}: full route failed, falling back to segments (${error.message})`);mode='mixed';for(let i=0;i<mapped.length-1;i++){const a=mapped[i].coord,b=mapped[i+1].coord;try{const segment=await routeGeometry([a,b]);if(road.length&&segment.length)segment.shift();road.push(...segment);}catch(segmentError){road.push(a,b);mode='approximate';}await delay(250);}}
    output.routes[route.id]={mode,road,accessFerry:accessFerries[route.id]||null};console.log(`${route.id}: ${road.length} points`);await delay(350);
  }
