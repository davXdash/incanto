...
function label(route,index){return `Route ${String(index+1).padStart(2,'0')} · ${route.title} · ${route.days}`}
...
function value(route,key,index){
  const meta=route.meta||{};
  if(key==='number')return `Route ${String(index+1).padStart(2,'0')}`;
  if(key==='hearts'){const count=HEARTS[route.id]??Math.max(0,Math.min(5,Number(meta.hearts||0)));return count?`${'♥'.repeat(count)}${'♡'.repeat(5-count)}`:'–'}
  ...
  if(key==='wine')return readable((meta.themes||[]).filter(item=>['wine','culinary','markets'].includes(item)))||((route.eyebrow||'').toLowerCase().includes('wein')?'Wein ist Teil der Route':'Kein ausgesprochener Weinfokus');
  ...
  if(key==='summer'){if(meta.summer_notes)return meta.summer_notes;if(meta.nrwSummerFit)return String(meta.nrwSummerFit).replaceAll('-',' ');return meta.summer_nrw===true?'Für die NRW-Sommerferien vorgesehen':'Saison im Detail prüfen'}
  if(key==='mobility')return String(meta.mobility||'nicht gesondert bewertet').replaceAll('-',' ');
  if(key==='heat')return String(meta.heat||'regional unterschiedlich').replaceAll('-',' ');
...
function init(routes){
  ...
  routes.forEach((route,index)=>{route.routeNumber=index+1;route.meta=route.meta||{};route.meta.hearts=HEARTS[route.id]??route.meta.hearts});
  const options=routes.map((route,index)=>`<option value="${route.id}">${label(route,index)}</option>`).join('');
  const oldA=first.value,oldB=second.value;first.innerHTML=options;second.innerHTML=options;first.value=routes.some(r=>r.id===oldA)?oldA:routes[0].id;second.value=routes.some(r=>r.id===oldB)?oldB:routes[Math.min(1,routes.length-1)].id;
